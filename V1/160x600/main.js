...
function animate() {
  ...
  tl.set([".copy2 .line, .copy3 .line, .copy4 .line"], { xPercent: -330 });
  tl.set([logo, img, bg1, bg2, bg3, bg4, copy1, copy2, copy3, copy4, cta], {force3D: true});
  tl.set([bg4], { y: 405 });

  tl.addLabel("frame1", 1)
    .to(logo, 1, { width: 96, x: 10.5, y: -1, ease: "power1.inOut" }, "frame1")
    .to(img, 1, { width: 134, height: 404, x: 13, y: 13, ease: 'Sine.easeOut' }, "frame1")
    .to(bg1, 1, { width: 134, height: 404, x: 0, y: 0, ease: 'Sine.easeOut'}, "frame1")
    .from(".copy1 .line", 0.5, { xPercent: -200, stagger: 0.2, ease: "power1.inOut" }, "frame1+=1")
